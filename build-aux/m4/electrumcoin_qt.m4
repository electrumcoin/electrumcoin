...
dnl Output: If qt version is auto, set electrumcoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$electrumcoin_qt_want_version" = xauto && test "x$electrumcoincon_qt_force" != xyes; then
    if test "x$electrumcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; litecoin-qt frontend will not be built])
    fi
    electrumcoin_enable_qt=no
    electrumcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$electrumcoin_enable_qt" != xno && test "x$electrumcoincon_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     electrumcoin_qt_want_version=$withval
     if test "x$electrumcoin_qt_want_version" = xyes; then
       electrumcoin_qt_force=yes
       electrumcoin_qt_want_version=auto
     fi
    ],
    [electrumcoin_qt_want_version=auto])
...
dnl Outputs: electrumcoin_enable_qt, electrumcoincon_enable_qt_dbus, electrumcoincon_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$electrumcoin_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    electrumcoin_enable_qt=yes
    electrumcoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      electrumcoin_enable_qt_test=no
    fi
    electrumcoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      electrumcoin_enable_qt_dbus=yes
    fi
  ...
  if test x$electrumcoin_enable_qt = xyes; then
    AC_MSG_RESULT([$electrumcoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$electrumcoin_enable_qt])
  fi
...
dnl Output: electrumcoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, electrumcoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [electrumcoin_cv_static_qt=yes],
      [electrumcoin_cv_static_qt=no])
    ])
])
