...
###   electrumcoincore.org and electrumcoincon.org and compares them.
...
WORKINGDIR="/tmp/electrumcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://electrumcoincore.org"
HOST2="https://electrumcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="electrumcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'electrumcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "electrumcoin.org failed to provide signature file, but electrumcoinconcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "electrumcoin.org and electrumcoinconcore.org signature files were not equal?"
