...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://electrumcoincore.org/bin/electrumcoincon-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/electrumcoin/electrumcoincon/issues/16599),
  [PR #16702](https://github.com/electrumcoin/electrumcoincon/pull/16702), and the
  `electrumcoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://electrumcoincore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/electrumcoin-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.electrumcoincore.org.  (#17596)
...
- The `electrumcoin-cli` utility used with the `-getinfo` parameter now
...
- The `electrumcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `electrumcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_electrumcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_electrumcoin` binary (hebasto)
...
- #18702 Fix ASLR for electrumcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate electrumcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest electrumcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_electrumcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (electrumcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone electrumcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken electrumcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from electrumcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.electrumcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to electrumcoin-wallet (hel-o)
...
- #16115 On electrumcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from electrumcoin-cli -getinfo (malevolent)
