Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://electrumcoincore.org/bin/electrumcoincon-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/electrumcoin/electrumcoincon/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `electrumcoind -help` output for these two options for
...
- The `testnet` field in `electrumcoin-cli -getinfo` has been renamed to
...
- #15639 electrumcoin-wallet tool: Drop libelectrumcoincon_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in electrumcoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "electrumcoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of electrumcoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from electrumcoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_electrumcoin to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_electrumcoin output (practicalswift)
...
- #15896 feature_filelock, interface_electrumcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_electrumcoin for expected errors (gertjaap)
...
- #16489 log: harmonize electrumcoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (electrumcoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example electrumcoin.conf (fanquake)
...
- #16080 build/doc: update electrumcoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in electrumcoin.conf (fanquake)
...
- #16621 add default electrumcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the electrumcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, electrumcoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/electrumcoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/electrumcoin/electrumcoincon/).
