Bitcoin Core version 0.12.0 is now available from:
  ...
  <https://electrumcoin.org/bin/electrumcoincon-core-0.12.0/>
...
(https://github.com/electrumcoin/bips/blob/master/bip-0125.mediawiki).
...
This allows running electrumcoind without having to do any manual configuration.
...
`electrumcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/electrumcoin/electrumcoincon/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
caching. A sample config for apache2 could look like:
    ...
    <Location /electrumcoinrpc>
        ...
        # optional bypass electrumcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< electrumcoinrpc:<password>
    </Location>
...
Restarting electrumcoind will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new electrumcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include electrumcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix electrumcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of electrumcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [electrumcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libelectrumcoinconsensus (Braydon Fuller)
